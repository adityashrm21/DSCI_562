...
# gaussian assumpton for Y|X not good due to restricted Y values.
```
...
<<<<<<< HEAD
 - Simply take the mean of the variance values.
=======
...
<<<<<<< HEAD
      After week 52, we need to come back to week 0
3. If we fit a Poisson regression model, would it make sense to estimate the variance in this way?
  - becuase we only have 1 param for poisson and exponential dist which is the mean.
...
flu <- flu %>% 
<<<<<<< HEAD
    mutate(q75_gauss = mean_lo + 1.96 * var_lo)
p + geom_line(
=======
...
<<<<<<< HEAD
# this is a non-parametric model for the qunatile where we use the values to get the quantile. If we assume a model fn for the quantiles, it will become a parametric assumption
=======
>>>>>>> upstream/master
```
...
# we did this without any assumption on model or dist
